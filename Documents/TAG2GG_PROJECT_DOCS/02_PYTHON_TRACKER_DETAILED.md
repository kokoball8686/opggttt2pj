...
또한 트래커의 외부 REST 통신은 오직 이 `matches` 테이블로의 결과 업로드(INSERT) 하나뿐이며, 과거 테스트용으로 존재했던 별도 진단용 로그 전송 기능(`test_log_sender.py`, `tracker_logs`)은 보안 및 프라이버시 원칙에 따라 완전히 제거되었다.
